...
// ip y puerto del servidor, /etc/hosts/text.txt, /etc/hosts/text2.txt
...
func handleDownload(conn net.Conn, filePath string, savePath string) {
	...
	fmt.Println("File downloaded succesfuly")
}
...
func handleClient(conn net.Conn) {
	...
	file, err := os.Open(fileName) //c::User/user/ fdas/dfa/
	defer file.Close()
	...
	if err == nil {
		...
		fmt.Printf("File %v doesnt exist", fileName)
